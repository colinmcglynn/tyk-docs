...
`access_rights`
Access rights can be defined either by the Dashboard or via an API, depending on the version of Tyk you are using. See the [Tutorials](/docs/try-out-tyk/tutorials/tutorials/) section for more details.
...
See [Auth Plugins](/docs/plugins/auth-plugins/) for more details.

`session_lifetime`
Overrides the global session lifetime, see [Physical Key Expiry](/docs/basic-config-and-security/security/your-apis/physical-key-expiry/) for details.
