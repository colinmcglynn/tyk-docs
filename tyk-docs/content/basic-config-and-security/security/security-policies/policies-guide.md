...
To apply the above policy to a key, we simply need to call the `/create` (or `/add`) endpoint in the Tyk Gateway API with a session object that has the `apply_policy_id` flag set to the name `default` (or whatever you named your policy).
...
* Create a Policy with the Dashboard API
* Create a Policy with the Gateway API
* Create a Policy with the Dashboard

See [Create a Security Policy](/docs/try-out-tyk/tutorials/create-security-policy/), and select the On-Premises tab from the tutorial section.
