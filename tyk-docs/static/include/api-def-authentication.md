...
Keys cannot be generated by Tyk for this method and need to use the `/tyk/keys/add` method with the `basic_auth_data` metadata set in the keys session configuration (see [Basic Authentication](/docs/basic-config-and-security/security/your-apis/basic-auth/) and the overview of the Session Object in this use case under [Key Management](/docs/tyk-gateway-api/key-session-object-details/)).
